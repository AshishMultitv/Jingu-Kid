...
class DownloadsViewController: UIViewController,UITableViewDelegate,UITableViewDataSource,UICollectionViewDataSource, UICollectionViewDelegate,UICollectionViewDelegateFlowLayout {
   ...
   if(Common.isInternetAvailable())
 {
    ...
    func isrechable()
    {
        ...
        let alert = UIAlertController(title: "Jingu Kid", message: "Internet connection is avilable now. Are you want to go online in this app.", preferredStyle: UIAlertControllerStyle.alert)
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell
    {
        ...
        let destinationURLForFile = URL(fileURLWithPath: path.appendingFormat("/\(dict.value(forKey: "id") as! String)\(Id!).mp4"))
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        if !fileManager.fileExists(atPath: filePath!) {
            ...
            progressimageview.animationImages = [image1,image2,image3,image4,image5,image6,image7,image8,image9,image10,image11,image12,image13,image14,image15]
            progressimageview.animationDuration = 1.0
            progressimageview.startAnimating()
            cell.addSubview(progressimageview)
         }
        ...
        let button: UIButton = UIButton()
        button.setImage(UIImage(named:"Cross"), for: .normal)
        button.frame = CGRect.init(x: cell.bannerimaheview.frame.size.width - 45, y: cell.bannerimaheview.frame.origin.y+5, width: 40, height: 40)
       button.addTarget(self, action: #selector(taptoremove), for: .touchUpInside)
    ...
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath)
    {
        ...
        let playerViewController = storyboard.instantiateViewController(withIdentifier: "PlayerViewController") as! PlayerViewController
            if(Common.isNotNull(object: (downloadarray.object(at: indexPath.row) as! NSDictionary).value(forKey: "des") as AnyObject?))
            {
        playerViewController.descriptiontext =  (downloadarray.object(at: indexPath.row) as! NSDictionary).value(forKey: "des") as! String
            ...
            if(Common.isNotNull(object: (downloadarray.object(at: indexPath.row) as! NSDictionary).value(forKey: "title") as AnyObject?))
            {
    ...
    func taptoremove(sender:UIButton)
   {
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath)
    {
        ...
        let downloadurl = NSURL(fileURLWithPath: path)
        let Id = (downloadarray.object(at: (indexPath.row)) as! NSDictionary).value(forKey: "id") as? String
        let filePath = downloadurl.appendingPathComponent("\(dict.value(forKey: "id") as! String)\(Id!).mp4")?.path
        let fileManager = FileManager.default
        if fileManager.fileExists(atPath: filePath!) {
